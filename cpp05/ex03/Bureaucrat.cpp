...
/*   Updated: 2025/04/01 10:13:04 by maheleni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::executeForm(AForm const & form) const noexcept {
    try {
        form.execute(*this);
        std::cout << this->getName() << " executed " << form.getName() << std::endl;
    }
    catch (std::exception & e) {
        std::cerr << this->getName() << " could not execute form because: " << e.what() << std::endl;
    }
    catch (const char * str) {
        std::cerr << this->getName() << " could not execute form because: " << str << std::endl;
    }
}
